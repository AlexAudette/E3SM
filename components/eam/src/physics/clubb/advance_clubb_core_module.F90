...
contains
  ...
  subroutine advance_clubb_core &
    (l_implemented, dt, fcor, sfc_elevation, hydromet_dim, & ! intent(in)
     ...
     wpthvp, wp2thvp, rtpthvp, wtrc_rtpthvp, thlpthvp, &                ! intent(inout)
...
! <--- h1g, 2012-06-14
    ...
    !water tracers (currently just dummy variables)
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset) :: &
      wtrc_rcm_frz, &
      wtrc_rcm_zm_frz, &
      wtrc_rtpthvp_zt_frz, &
    ...
    if (ipdf_call_placement == ipdf_pre_advance_fields &
      ...
      call pdf_closure_driver(dt, hydromet_dim, wprtp, wtrc_wprtp, & ! Intent(in)
                              thlm, wpthlp, rtp2, wtrc_rtp2, & ! Intent(in)
                              rtp3, wtrc_rtp3, & ! Intent(in)
                              thlp2, thlp3, rtpthlp, & ! Intent(in)
                              wtrc_rtpthlp, wp2, & ! Intent(in)
                              ...
                              rtpthvp, wtrc_rtpthvp, thlpthvp, rc_coef, & ! Intent(out)
                              rcm_in_layer, cloud_cover, & ! Intent(out)
                              ...
                              rtm_zm, wtrc_rtm_zm, thlm_zm, & ! Intent(out)
                              rcm_zm, wtrc_rcm_zm, & ! Intent(out)
    ...
    end if ! ipdf_call_placement == ipdf_pre_advance_fields
    ...
    end if ! ipdf_call_placement == ipdf_post_advance_fields
...
! End Vince Larson's commenting.
    ...
    if (abs(gr%zm(1) - sfc_elevation) <= abs(gr%zm(1) + sfc_elevation)*eps/2) then
      ...
      call calc_surface_varnce(upwp_sfc, vpwp_sfc, wpthlp_sfc, wprtp_sfc, &      ! intent(in)
                               wtrc_wprtp_sfc, um(2), vm(2), Lscale_up(2), wpsclrp_sfc, &      ! intent(in)
                               wp2_splat(1), tau_zm(1), &      ! intent(in)
                               wp2(1), up2(1), vp2(1), &      ! intent(out)
                               thlp2(1), rtp2(1), wtrc_rtp2(1, 1:wtrc_nwset), &
                               rtpthlp(1), wtrc_rtpthlp(1, 1:wtrc_nwset), &      ! intent(out)
      ...
      !water tracers
      wtrc_rtp2(1, :) = rt_tol**2
      wtrc_rtpthlp(1, :) = 0.0_core_rknd
    ...
    call advance_xm_wpxp(dt, sigma_sqd_w, wm_zm, wm_zt, wp2, & ! intent(in)
                         ...
                         tau_C6_zm, Skw_zm, wp2rtp, rtpthvp, wtrc_rtpthvp, rtm_forcing, wtrc_rtm_forcing, & ! intent(in)
                         wprtp_forcing, wtrc_wprtp_forcing, rtm_ref, wp2thlp, thlpthvp, & ! intent(in)
                         thlm_forcing, wpthlp_forcing, thlm_ref, & ! intent(in)
                         rho_ds_zm, rho_ds_zt, invrs_rho_ds_zm, & ! intent(in)
                         invrs_rho_ds_zt, thv_ds_zm, rtp2, wtrc_rtp2, thlp2, & ! intent(in)
                         ...
                         rtm, wtrc_rtm, wprtp, wtrc_wprtp, thlm, wpthlp, & ! intent(inout)
    ...
    do m = 2, wtrc_nwset
      do k = 1, gr%nz
        wtrc_rcm(k, m) = wtrc_ratio(m, wtrc_rcm(k, m), wtrc_rcm(k, 1))*rcm(k)
      end do
    end do

    wtrc_rcm(:, 1) = rcm
    !water tracers
    ! call wtrc_clip_rcm( wtrc_rtm, 'wtrc rtm < rcm in advance_xm_wpxp', & ! intent)in)
    !                 wtrc_rcm )
    ...
    call advance_xp2_xpyp(tau_zm, wm_zm, rtm, wtrc_rtm, wprtp, wtrc_wprtp, thlm, & ! intent(in)
                          wpthlp, wpthvp, um, vm, wp2, wp2_zt, & ! intent(in)
                          wp3, upwp, vpwp, sigma_sqd_w, Skw_zm, & ! intent(in)
                          wprtp2, wpthlp2, wprtpthlp, & ! intent(in)
                          Kh_zt, rtp2_forcing, wtrc_rtp2_forcing, thlp2_forcing, & ! intent(in)
                          rtpthlp_forcing, wtrc_rtpthlp_forcing, rho_ds_zm, rho_ds_zt, & ! intent(in)
                          ...
                          wp2_splat, & ! intent(in)
                          rtp2, wtrc_rtp2, thlp2, rtpthlp, wtrc_rtpthlp, up2, vp2, & ! intent(inout)
                          sclrp2, sclrprtp, sclrpthlp)              ! intent(inout)

    !HACK -JN:
    !--------
    !NOTE:  If the H2O water mass variance is
    !rt_tol**2, then that means it was either clipped
    !or skipped over in the previous call, and thus in
    !order to avoid large differences in the variances,
    !all other water tracers should be set (or "clipped")
    !to the same value.  Ideally, one should go into
    !the advance_xp2_xpyp subroutine itself and figure
    !out exactly where this mismatch is occuring, but for
    !now this will do. -JN
    do k = 1, gr%nz, 1
      if (wtrc_rtp2(k, 1) == rt_tol**2) then
        wtrc_rtp2(k, :) = rt_tol**2
      end if
    end do
    !--------
    ...
    if (l_advance_xp3) then
      ...
      do m = 1, wtrc_nwset
        ...
        wtrc_rtp2_zt(:, m) = max(zm2zt(wtrc_rtp2(:, m)), rt_tol**2)
        wtrc_rtp3(:, m) = xp3_LG_2005_ansatz(Skw_zt, wtrc_wprtp_zt(:, m), wp2_zt, &
                                             wtrc_rtp2_zt(:, m), sigma_sqd_w_zt, &
                                             rt_tol)
      end do
    ...
    if (ipdf_call_placement == ipdf_post_advance_fields &
      ...
      call pdf_closure_driver(dt, hydromet_dim, wprtp, wtrc_wprtp, & ! Intent(in)
                              thlm, wpthlp, rtp2, wtrc_rtp2, rtp3, wtrc_rtp3, & ! Intent(in)
                              thlp2, thlp3, rtpthlp, wtrc_rtpthlp, wp2, & ! Intent(in)
                              wp3, wm_zm, wm_zt, & ! Intent(in)
                              um, up2, upwp, & ! Intent(in)
                              vm, vp2, vpwp, & ! Intent(in)
                              p_in_Pa, exner, & ! Intent(in)
  ...
  !=============================================================================
  subroutine pdf_closure_driver(dt, hydromet_dim, wprtp, wtrc_wprtp, & ! Intent(in)
                                thlm, wpthlp, rtp2, wtrc_rtp2, &
                                rtp3, wtrc_rtp3, & ! Intent(in)
                                thlp2, thlp3, rtpthlp, &
                                wtrc_rtpthlp, wp2, & ! Intent(in)
                                wp3, wm_zm, wm_zt, & ! Intent(in)
    ...
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(inout) :: &
      wtrc_rtm    ! total water mixing ratio, r_t (thermo. levels) [kg/kg]
...
! <--- h1g, 2012-06-14
    ...
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(out) :: &
      wtrc_rcm, & ! mean r_c (thermodynamic levels)        [kg/kg]
      wtrc_rtpthvp ! < r_t' th_v' > (momentum levels)       [kg/kg K]
    ...
    do m = 1, wtrc_nwset
      ...
      ! wtrc_rtpthvp(:,m) = 0.0_core_rknd
      ! wtrc_rtpthvp_zt(:,m) = 0.0_core_rknd
    end do
    ...
    do m = 1, wtrc_nwset
      wtrc_rtp2_zt(:, m) = max(zm2zt(wtrc_rtp2(:, m)), rt_tol**2)   ! Positive def. quantity
      do k = 1, gr%nz
        wtrc_rtpthlp_zt(k, m) = rtpthlp_zt(k)*wtrc_ratio(m, wtrc_rtpthlp(k, m), wtrc_rtpthlp(k, 1))!zm2zt( wtrc_rtpthlp(:,m) )
      end do
    end do
    ...
    call wtrc_pdf_closure &
      (hydromet_dim, p_in_Pa, exner, thv_ds_zt, & ! intent(in)
       ...
       rtm, rtp2_zt, &
       zm2zt(wprtp), Skwtrc_zt, wtrc_rtm, wtrc_rtp2_zt, wtrc_wprtp_zt, & ! intent(in)
       thlm, thlp2_zt, zm2zt(wpthlp), & ! intent(in)
       um, zm2zt(up2), zm2zt(upwp), & ! intent(in)
       vm, zm2zt(vp2), zm2zt(vpwp), & ! intent(in)
       rtpthlp_zt, wtrc_rtpthlp_zt, & ! intent(in)
       ...
       rcm, wtrc_rcm, wpthvp_zt, wp2thvp, rtpthvp_zt, wtrc_rtpthvp_zt, & ! intent(out)
       thlpthvp_zt, wprcp_zt, wp2rcp, rtprcp_zt, wtrc_rtprcp_zt, & ! intent(out)
       thlprcp_zt, rcp2_zt, wtrc_rcp2_zt, & ! intent(out)
    ...
    if (l_call_pdf_closure_twice) then
      ...
      call wtrc_pdf_closure &
        (hydromet_dim, p_in_Pa_zm, exner_zm, thv_ds_zm, & ! intent(in)
         ...
         rtm_zm, rtp2, wprtp, Skwtrc_zm, wtrc_rtm_zm, wtrc_rtp2, wtrc_wprtp, & ! intent(in)
         thlm_zm, thlp2, wpthlp, & ! intent(in)
         zt2zm(um), up2, upwp, & ! intent(in)
         zt2zm(vm), vp2, vpwp, & ! intent(in)
         rtpthlp, wtrc_rtpthlp, & ! intent(in)
         ...
         rcm_zm, wtrc_rcm_zm, wpthvp, wp2thvp_zm, rtpthvp, wtrc_rtpthvp, & ! intent(out)
         thlpthvp, wprcp, wp2rcp_zm, rtprcp, wtrc_rtprcp, & ! intent(out)
         thlprcp, rcp2, wtrc_rcp2, & ! intent(out)
         uprcp, vprcp, & ! intent(out)
      ...
      if (ircp2 > 0) then
        ...
        do m = 1, wtrc_nwset
          wtrc_rcp2(:, m) = max(zt2zm(wtrc_rcp2_zt(:, m)), zero_threshold)
        end do
#ifndef CLUBB_CAM
        rcp2(gr%nz) = zero
        do m = 1, wtrc_nwset
      ...
      do m = 1, wtrc_nwset
        wtrc_rtprcp(:, m) = zt2zm(wtrc_rtprcp_zt(:, m))
        wtrc_rtprcp(gr%nz, m) = 0.0_core_rknd
        wtrc_rtpthvp(:, m) = zt2zm(wtrc_rtpthvp_zt(:, m))
        wtrc_rtpthvp(gr%nz, m) = 0.0_core_rknd
      end do
    ...
    !-----------------------------
    !water tracers: Calculate Rfix (needed for post-trapezoidal error correction)
    !-----------------------------
    wtrc_Rfix_liq(:, :) = 1.0
    wtrc_Rfix_vap(:, :) = 1.0
    do k = 1, gr%nz, 1
      do m = 1, wtrc_nwset
        !liquid:
        wtrc_Rfix_liq(k, m) = wtrc_ratio(m, &
                                         wtrc_rcm(k, m), wtrc_rcm(k, 1))
        !vapor:
        wtrc_Rfix_vap(k, m) = wtrc_ratio(m, &
    ...
    if (l_trapezoidal_rule_zt) then
      call trapezoidal_rule_zt &
        (l_call_pdf_closure_twice, & ! intent(in)
         ...
         pdf_params_zm, wtrc_rcm, wtrc_rcm_zm)                                ! intent(inout)
    ...
    if (l_trapezoidal_rule_zm) then
      call trapezoidal_rule_zm &
        (wpthvp_zt, thlpthvp_zt, rtpthvp_zt, & ! intent(in)
         wpthvp, thlpthvp, rtpthvp, wtrc_rtpthvp_zt, wtrc_rtpthvp)           ! intent(inout)
    end if ! l_trapezoidal_rule_zm

    !--------------------------------------
    !water tracers:  fix trapezoidal errors
    !--------------------------------------
    do k = 1, gr%nz, 1
      do m = 1, wtrc_nwset
        if (wtrc_rcm(k, m) .gt. 10.0) then
          write (*, *) 'wiso liquid-error post-trap', wtrc_rcm(k, m), &
            wtrc_rcm(k, 1), k, m
          !Eliminate error:
          wtrc_rcm(k, m) = wtrc_Rfix_liq(k, m)*rcm(k)
        end if
        if (wtrc_rtm(k, m) .gt. 2.0) then
          write (*, *) 'HDO vapor-error post-trap', wtrc_rtm(k, m), &
            wtrc_rtm(k, 1), k, m
          !Eliminate error:
          wtrc_rtm(k, m) = wtrc_Rfix_vap(k, m)*rtm(k)
        end if
      end do
    end do
    ...
    wtrc_rcm(:, 1) = rcm
    !water tracers
    ! call wtrc_clip_rcm( wtrc_rtm, 'wtrc rtm < rcm after pdf_closure', & ! intent (in)
    ...
    if (l_rcm_supersat_adj) then
      ...
      do k = 2, gr%nz
        if (rel_humidity(k) > 1.0_core_rknd) then
          ...
          !water tracers
          !-------------
          wtrc_cond = ((rtm(k) - rcm(k)) - rsat(k)) !save condensation amount
          ovapor = wtrc_rtm(k, 1) - wtrc_rcm(k, 1) !save original vapor
          !-------------
          ...
          do m = 1, wtrc_nwset
            R = wtrc_ratio(m, &
                           (wtrc_rtm(k, m) - wtrc_rcm(k, m)), &
                           ovapor)
            !conserve ratio during phase change:
            wtrc_rcm(k, m) = wtrc_rcm(k, m) + R*wtrc_cond
          end do
  ...
  subroutine setup_clubb_core &
    ...
    if (iiPDF_type < iiPDF_ADG1 .or. iiPDF_type > iiPDF_LY93) then
      ...
      err_code = clubb_fatal_error
      return
    end if ! iiPDF_type < iiPDF_ADG1 or iiPDF_type > iiPDF_lY93
    ...
    if (iiPDF_type == iiPDF_ADG2) then
      if (.not. l_input_fields) then
        ...
        err_code = clubb_fatal_error
        return
      end if ! .not. l_input_fields
    end if ! iiPDF_type == iiPDF_ADG2
    ...
    end if ! iiPDF_type == iiPDF_TSDADG
    ...
    if (l_predict_upwp_vpwp) then
      ...
      if (l_explicit_turbulent_adv_wpxp) then
        ...
        err_code = clubb_fatal_error
        return
      end if ! l_explicit_turbulent_adv_wpxp
...
! setup  prognostic_variables
  ...
  subroutine trapezoidal_rule_zt &
    (l_call_pdf_closure_twice, & ! intent(in)
     wprtp2, wpthlp2, & ! intent(inout)
     wprtpthlp, cloud_frac, ice_supersat_frac, & ! intent(inout)
     rcm, wp2thvp, & ! intent(inout)
     wpsclrprtp, wpsclrp2, & ! intent(inout)
     wpsclrpthlp, pdf_params, & ! intent(inout)
     wprtp2_zm, wpthlp2_zm, & ! intent(inout)
     wprtpthlp_zm, cloud_frac_zm, & ! intent(inout)
     ice_supersat_frac_zm, rcm_zm, wp2thvp_zm, & ! intent(inout)
    ...
    !water tracers
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(inout), optional :: &
      wtrc_rcm
    ...
    integer :: i
    integer :: m        !loop control variable for water tracers
  ...
  subroutine trapezoidal_rule_zm &
    ...
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(in), optional :: &
      wtrc_rtpthvp_zt     ! r_t' th_v' (on thermo. grid)     [(kg K)/kg]
    ...
    if (present(wtrc_rtpthvp_zt)) then

      do m = 1, wtrc_nwset

        wtrc_rtpthvp(:, m) = trapezoid_zm(wtrc_rtpthvp(:, m), wtrc_rtpthvp_zt(:, m))
      end do
    end if
  ...
  subroutine compute_cloud_cover &
    (pdf_params, cloud_frac, rcm, & ! intent(in)
     wtrc_rcm, cloud_cover, rcm_in_layer, &
     wtrc_rcm_in_layer)    ! intent(out)
    !
    ! Description:
    ...
    ! water tracers
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(out) :: &
      wtrc_rcm_in_layer !water tracer liquid water in cloud layer [kg/kg]
