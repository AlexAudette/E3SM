...
contains
  ...
  subroutine calc_surface_varnce(upwp_sfc, vpwp_sfc, wpthlp_sfc, wprtp_sfc, &
                                 wtrc_wprtp_sfc, um_sfc, vm_sfc, Lscale_up_sfc, wpsclrp_sfc, wp2_splat_sfc, tau_zm_sfc, &
                                 wp2_sfc, up2_sfc, vp2_sfc, &
                                 thlp2_sfc, rtp2_sfc, wtrc_rtp2_sfc, rtpthlp_sfc, &
                                 wtrc_rtpthlp_sfc, &
                                 sclrp2_sfc, &
                                 sclrprtp_sfc, &
                                 sclrpthlp_sfc)
    ...
    if (l_andre_1978) then
      ...
      if (abs(wtrc_rtp2_sfc(1) - rtp2_sfc) .gt. 1.e-24) then
        write (iulog, *) 'wtrc_rtp2_sfc(1) = ', wtrc_rtp2_sfc(1), &
          'rtp2_sfc = ', rtp2_sfc
        call endrun('wtrc_rtp2_sfc(1) error')
      end if
...
!             = sclr_var_coef * a * ( wpsclrp_sfc(i) / uf )**2
    ...
    if (clubb_at_least_debug_level(2)) then
      ...
      end if ! err_code == clubb_fatal_error
