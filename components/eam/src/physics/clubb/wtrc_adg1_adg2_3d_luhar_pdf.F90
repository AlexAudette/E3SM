! $Id$
!===============================================================================
module wtrc_adg1_adg2_3d_luhar_pdf
...
contains
  ...
  !=============================================================================
  subroutine ADG1_pdf_driver(wm, rtm, wtrc_rtm, thlm, um, vm, & ! In
                             ...
                             mixt_frac, alpha_rt, alpha_wtrc, alpha_thl, & ! Out
                             alpha_u, alpha_v, & ! Out
    ...
    ! Calculate the PDF component means and variances of water tracers.
    if (trace_water) then
      do i = 1, wtrc_nwset
        call ADG1_ADG2_responder_params(wtrc_rtm(:, i), wtrc_rtp2(:, i), & ! In
                                        wp2, sqrt_wp2, wtrc_wprtp(:, i), & ! In
                                        w_1_n, w_2_n, mixt_frac, & ! In
                                        sigma_sqd_w, rt_tol, & ! In
                                        wtrc_rt_1(:, i), wtrc_rt_2(:, i), & ! Out
                                        varnce_wtrc_1(:, i), & ! Out
                                        varnce_wtrc_2(:, i), & ! Out
                                        alpha_wtrc(:, i))              ! Out
      end do ! i=1, sclr_dim
  ...
  !=============================================================================
  subroutine ADG2_pdf_driver(wm, rtm, wtrc_rtm, thlm, wp2, rtp2, wtrc_rtp2, thlp2, & ! In
                             Skw, wprtp, wtrc_wprtp, wpthlp, sqrt_wp2, & ! In
                             sclrm, sclrp2, wpsclrp, l_scalar_calc, & ! In
                             w_1, w_2, rt_1, wtrc_1, rt_2, wtrc_2, thl_1, thl_2, & ! Out
                             varnce_w_1, varnce_w_2, varnce_rt_1, varnce_wtrc_1, & ! Out
                             varnce_rt_2, varnce_wtrc_2, varnce_thl_1, varnce_thl_2, & ! Out
                             mixt_frac, alpha_rt, alpha_wtrc, alpha_thl, & ! Out
    ...
    ! Input Variables
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(in) :: &
      wtrc_rtm, & ! Mean of total water mixing ratio              [kg/kg]
      wtrc_rtp2, & ! Variance of r_t
      wtrc_wprtp ! Covariance of w and r_t                       [(kg/kg)(m/s)]
    ...
    ! Output Variables
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset), intent(out) :: &
      wtrc_1, & ! Mean of r_t (1st PDF component)                  [kg/kg]
      wtrc_2, & ! Mean of r_t (2nd PDF component)                  [kg/kg]
      varnce_wtrc_1, & ! Variance of r_t (1st PDF component)          [kg^2/kg^2]
      varnce_wtrc_2, & ! Variance of r_t (2nd PDF component)          [kg^2/kg^2]
      alpha_wtrc ! Factor relating to normalized variance for r_t       [-]
    ...
    if (trace_water) then
      do m = 1, wtrc_nwset
        call ADG1_ADG2_responder_params(wtrc_rtm(:, m), wtrc_rtp2(:, m), & ! In
                                        wp2, sqrt_wp2, wtrc_wprtp(:, m), & ! In
                                        w_1_n, w_2_n, mixt_frac, & ! In
                                        sigma_sqd_w, rt_tol, & ! In
                                        wtrc_1(:, m), wtrc_2(:, m), & ! Out
                                        varnce_wtrc_1(:, m), & ! Out
                                        varnce_wtrc_2(:, m), & ! Out
                                        alpha_wtrc(:, m))          ! Out
      end do
  ...
  subroutine Luhar_3D_pdf_driver(wm, rtm, thlm, wp2, rtp2, thlp2, & ! In
    ...
    do k = 1, gr%nz, 1
      ...
      if ((abs(Skw(k)) >= abs(Skthl(k))) &
        ...
        !-------------
        !  do m=1,wtrc_nwset
        !    ! Solve for the wtrc PDF

        !   call backsolve_Luhar_params( Skw(k), Skwtrc(k,m),           & ! In
        !                              big_m_w(k), mixt_frac(k),  & ! In
        !                              big_m_wtrc(k,m), small_m_wtrc(k,m) ) ! Out

        ! call close_Luhar_pdf( 1, wtrc_rtm(k,m), wtrc_rtp2(k,m), mixt_frac(k),      & ! In
        !                       small_m_wtrc(k,m), wtrc_wprtp(k,m), rt_tol**2,    & ! In
        !                       sigma_sqd_wtrc_1(k), sigma_sqd_wtrc_2(k),  & ! Out
        !                       varnce_wtrc_1(k,m), varnce_wtrc_2(k,m),        & ! Out
        !                       wtrc_1_n(k), wtrc_2_n(k), wtrc_1(k,m), wtrc_2(k,m) ) ! Out
...
!          x_2_n = - ( wpxp / ( sqrt( wp2 ) * sqrt( xp2 ) ) ) / w_1_n
    ...
    end where  ! Widths of w non-zero
...
end module wtrc_adg1_adg2_3d_luhar_pdf
