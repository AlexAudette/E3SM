...
module advance_xp2_xpyp_module
  ...
  integer, parameter, private :: &
    ...
    xp2_xpyp_wtrc = 11 ! Named constant for wtrc solves

contains

  !=============================================================================
  subroutine advance_xp2_xpyp(tau_zm, wm_zm, rtm, wtrc_rtm, wprtp, wtrc_wprtp, thlm, & ! In
                              wpthlp, wpthvp, um, vm, wp2, wp2_zt, & ! In
                              wp3, upwp, vpwp, sigma_sqd_w, Skw_zm, & ! In
                              wprtp2, wpthlp2, wprtpthlp, & ! In
                              Kh_zt, rtp2_forcing, wtrc_rtp2_forcing, thlp2_forcing, & ! In
                              rtpthlp_forcing, wtrc_rtpthlp_forcing, rho_ds_zm, rho_ds_zt, & ! In
                              ...
                              wp2_splat, & ! In
                              rtp2, wtrc_rtp2, thlp2, rtpthlp, wtrc_rtpthlp, up2, vp2, & ! Inout
    ...
    real(kind=core_rknd), dimension(gr%nz, wtrc_nwset) :: &
      wtrc_term_wprtp2_explicit_zm, &
      wtrc_term_wprtp2_explicit, &
      wtrc_term_wprtpthlp_explicit, &
      wtrc_term_wprtpthlp_explicit_zm
    ...
    if (l_explicit_turbulent_adv_xpyp) then
      ...
      if (iiPDF_type == iiPDF_ADG1) then
        ...
        do m = 1, wtrc_nwset
          do k = 1, gr%nz
            wtrc_wprtp_zt(k, m) = wprtp_zt(k)*wtrc_ratio(m, wtrc_wprtp(k, m), wtrc_wprtp(k, 1))!zm2zt( wtrc_wprtp(:,m) )
          end do
        end do
        ...
        do m = 1, wtrc_nwset
          wtrc_term_wprtp2_explicit(:, m) &
            = (one - one_third*beta)*a1_zt**2*wtrc_wprtp_zt(:, m)**2 &
              *wp3_on_wp2_zt/wp2_zt
        end do
        ...
        if (l_upwind_xpyp_ta) then
          ...
          do m = 1, wtrc_nwset
            wtrc_term_wprtp2_explicit_zm(:, m) &
              = (one - one_third*beta)*a1**2*wtrc_wprtp(:, m)**2*wp3_on_wp2/wp2
          end do
        ...
        do m = 1, wtrc_nwset
          do k = 1, gr%nz
            R = wtrc_ratio(m, wtrc_wprtp_zt(k, m), wtrc_wprtp_zt(k, 1))
            wtrc_term_wprtpthlp_explicit(k, m) = R*term_wprtpthlp_explicit(k)
            ! if (m .eq. 2) then
            !   if (abs(R - 0.5_core_rknd) .gt. 0.001) then
            !     write(iulog, *) 'wtrc_ratio',m , k, wtrc_ratio(m, wtrc_wprtp_zt(k,m), wtrc_wprtp_zt(k,1))
            !   endif
            ! endif
          end do
          !   wtrc_term_wprtpthlp_explicit(:,m) &
          ! = ( one - one_third * beta ) * a1_zt**2 * wtrc_wprtp_zt(:,m) * wpthlp_zt &
          !     * wp3_on_wp2_zt / wp2_zt
        end do
        ...
        if (l_upwind_xpyp_ta) then
          ...
          do m = 1, wtrc_nwset
            wtrc_term_wprtpthlp_explicit_zm(:, m) &
              = (one - one_third*beta)*a1**2*wtrc_wprtp(:, m)*wpthlp &
                *wp3_on_wp2/wp2
          end do
    ...
    do m = 1, wtrc_nwset
      ...
      call xp2_xpyp_rhs(xp2_xpyp_wtrc, dt, l_iter, & ! In
                        coef_wprtp2_implicit, coef_wprtp2_implicit_zm, & ! In
                        wtrc_term_wprtp2_explicit(:, m), wtrc_term_wprtp2_explicit_zm(:, m), & ! In
                        sgn_t_vel_rtp2, wtrc_wprtp(:, m), wtrc_wprtp(:, m), & ! In
                        wtrc_rtm(:, m), wtrc_rtm(:, m), wtrc_rtp2(:, m), wtrc_rtp2_forcing(:, m), & ! In
      ...
      call xp2_xpyp_solve(xp2_xpyp_rtp2, 1, & ! Intent(in)
                          wtrc_rhs(:, 1, m), wtrc_lhs(:, :, m), wtrc_rtp2(:, m))    ! Intent(inout)
    end do
    ...
    ! call xp2_xpyp_rhs( xp2_xpyp_wtrc, dt, l_iter, & ! In
    !                   coef_wprtpthlp_implicit, coef_wprtpthlp_implicit_zm, &!In
    !                   wtrc_term_wprtpthlp_explicit(:,m),  wtrc_term_wprtpthlp_explicit_zm(:,m), &!In
    !                   sgn_t_vel_rtpthlp, wtrc_wprtp(:,m), wpthlp, & ! In
    !                   wtrc_rtm(:,m), thlm, wtrc_rtpthlp(:,m), wtrc_rtpthlp_forcing(:,m), & ! In
    !                   rho_ds_zm, rho_ds_zt, invrs_rho_ds_zm, & ! In
    !                   C2rtthl_1d, tau_zm, zero_threshold, & ! In
    !                   wtrc_rhs(:,:,m) ) ! Out
    ...
    do m = 1, wtrc_nwset
      do k = 1, gr%nz
        wtrc_rhs(k, 1, m) = rhs(k, 1)*wtrc_ratio(m, wtrc_rtpthlp(k, m), wtrc_rtpthlp(k, 1))
      end do
    end do
    ! do m = 1, wtrc_nwset
    !                     ! Solve the tridiagonal system
    !   call xp2_xpyp_solve( xp2_xpyp_rtpthlp, 1, & ! Intent(in)
    !                       wtrc_rhs(:,:,m), wtrc_lhs(:,:,m), wtrc_rtpthlp(:,m) )    ! Intent(inout)
    ! enddo
    ...
    do m = 1, wtrc_nwset
      do k = 1, gr%nz
        wtrc_rtpthlp(k, m) = rtpthlp(k)*wtrc_ratio(m, wtrc_rhs(k, 1, m), wtrc_rhs(k, 1, 1))
      end do
    end do
    ...
    wtrc_rtpthlp_tmp = wtrc_rtpthlp
    do m = 1, wtrc_nwset
      ! call clip_covar( xp2_xpyp_sclrpthlp, l_first_clip_ts,  &     ! Intent(in)
      !               l_last_clip_ts, dt, wtrc_rtp2(:,m), thlp2,  &  ! Intent(in)
      !               wtrc_rtpthlp(:,m), wtrc_rtpthlp_chnge(:,m) )        ! Intent(inout)

      do k = 1, gr%nz
        R = wtrc_ratio(m, wtrc_rtpthlp(k, m), wtrc_rtpthlp_tmp(k, 1))

        wtrc_rtpthlp(k, m) = rtpthlp(k)*R
      end do
