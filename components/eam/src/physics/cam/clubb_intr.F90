...
!++ag
  use water_tracer_vars, only: trace_water, wtrc_nwset, &
                               wtrc_iatype, iwspec, &
...
!-------------------------------------------------------------------------------
    ...
    if (clubb_do_adv) then
      ...
      call cnst_add(trim(cnst_names(1)), 0._r8, 0._r8, 0._r8, ixthlp2, &
                    longname='second moment vertical velocity', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(2)), 0._r8, 0._r8, 0._r8, ixrtp2, &
                    longname='second moment rtp', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(3)), 0._r8, 0._r8, -999999._r8, ixrtpthlp, &
                    longname='covariance rtp thlp', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(4)), 0._r8, 0._r8, -999999._r8, ixwpthlp, &
                    longname='CLUBB heat flux', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(5)), 0._r8, 0._r8, -999999._r8, ixwprtp, &
                    longname='CLUBB moisture flux', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(6)), 0._r8, 0._r8, 0._r8, ixwp2, &
                    longname='CLUBB wp2', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(7)), 0._r8, 0._r8, -999999._r8, ixwp3, &
                    longname='CLUBB 3rd moment vert velocity', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(8)), 0._r8, 0._r8, 0._r8, ixup2, &
                    longname='CLUBB 2nd moment u wind', cam_outfld=.false.)
      call cnst_add(trim(cnst_names(9)), 0._r8, 0._r8, 0._r8, ixvp2, &
                    longname='CLUBB 2nd moment v wind', cam_outfld=.false.)
    end if
...
!-------------------------------------------------------------------------------
    ...
    wtrc_rtp2_idx = pbuf_get_index('wtrc_RTP2_nadv')
    wtrc_rcm_idx = pbuf_get_index('WTRC_RCM')
    wtrc_wprtp_idx = pbuf_get_index('wtrc_WPRTP_nadv')
    wtrc_rtpthlp_idx = pbuf_get_index('wtrc_RTTH_nadv')
    wtrc_rtpthvp_idx = pbuf_get_index('wtrc_RTTV_nadv')
!--ag
...
!PMA gustiness output fields
    ...
    if (is_first_step()) then
      ...
      if (trace_water) then
        ...
        call pbuf_set_field(pbuf2d, wtrc_rtp2_idx, real(rt_tol**2, kind=r8))
        call pbuf_set_field(pbuf2d, wtrc_rtpthlp_idx, 0.0_r8)
        call pbuf_set_field(pbuf2d, wtrc_rtpthvp_idx, 0.0_r8)
        call pbuf_set_field(pbuf2d, wtrc_rcm_idx, 0.0_r8)
      end if
...
!=====================================================================================
    ...
    !water tracers:
    real(r8) :: wtrc_rtm_forcing(pverp, wtrc_nwset)   !wtrc_rt forcing (may not actually be used)
    real(r8) :: wtrc_wprtp_forcing(pverp, wtrc_nwset) !wtrc_wprtp forcing (may not actually be used)
    real(r8) :: wtrc_rtp2_forcing(pverp, wtrc_nwset)  !wtrc_rt^2 forcing
    real(r8) :: wtrc_rtpthlp_forcing(pverp, wtrc_nwset) !wtrc_rt'thl' forcing
    real(r8) :: wtrc_rtp3_in(pverp, wtrc_nwset)      ! wtrc_r_t'^3 (unused)       [(kg/kg)^3]
    real(r8) :: wtrc_wprtp_in(pverp, wtrc_nwset)      ! w'wtrc_rt' [(kg/kg)(m/s)]
    real(r8) :: wtrc_rtp2_in(pverp, wtrc_nwset)       ! wtrc_rt'wtrc_rt' [(kg/kg)^2]
    real(r8) :: wtrc_rtm_in(pverp, wtrc_nwset)        ! CLUBB input of water tracer total water/vapor [kg/kg]
    real(r8) :: wtrc_rtpthlp_in(pverp, wtrc_nwset)      ! wtrc_rt'thl' [(kg/kg) K]
    real(r8) :: wtrc_rcm_inout(pverp, wtrc_nwset)       ! CLUBB output of water tracer cloud liquid    [kg/kg]
    real(r8) :: wtrc_rtpthvp_inout(pverp, wtrc_nwset)       ! CLUBB output of water tracer cloud liquid    [kg/kg]
...
!++ag
    ...
    real(r8), pointer, dimension(:, :, :) :: wtrc_rtpthvp !water tracer moisture/theta_v covariance [ (kg/kg) K]
    real(r8), pointer, dimension(:, :, :) :: wtrc_rcm!water tracer moisture/theta_v covariance [ (kg/kg) K]
    real(r8) :: wtrc_rtm(pcols, pverp, wtrc_nwset)      !water tracer "toal water" (assumes all vapor) [kg/kg]
    real(r8) :: wtrc_rtm_old(pcols, pverp, wtrc_nwset)   !wtrc_rt forcing (may not actually be used)
    ...
    !dummy precipitation output from rates
    !Precipitation output (needed due to microphysics iterations):
    real(r8) precr(pcols, wtrc_nwset)
    real(r8) preci(pcols, wtrc_nwset)
!--ag
...
! numbers are coefficients of the empirical equation
    ...
    call pbuf_get_field(pbuf, wtrc_rtpthvp_idx, wtrc_rtpthvp, start=(/1, 1, 1, itim_old/), kount=(/pcols, pverp, wtrc_nwset, 1/))
    call pbuf_get_field(pbuf, wtrc_rcm_idx, wtrc_rcm, start=(/1, 1, 1, itim_old/), kount=(/pcols, pverp, wtrc_nwset, 1/))
...
!PMA adds fields for gustiness
    ...
    if (micro_do_icesupersat) then
      ...
      call ice_macro_tend(naai(:ncol, top_lev:pver), state1%t(:ncol, top_lev:pver), &
                          state1%pmid(:ncol, top_lev:pver), state1%q(:ncol, top_lev:pver, 1), &
                          state1%q(:ncol, top_lev:pver, ixcldice), state1%q(:ncol, top_lev:pver, ixnumice), &
                          latsub, hdtime, stend(:ncol, top_lev:pver), qvtend(:ncol, top_lev:pver), &
                          qitend(:ncol, top_lev:pver), initend(:ncol, top_lev:pver))
      ...
      do i = 1, ncol ! loop over columns
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            ...
            R = wtrc_ratio(m, wtrc_rtm(i, k, m), wtrc_rtm(i, k, 1))
            wtrc_wprtp(i, k, m) = R*wprtp(i, k)
            wtrc_rtp2(i, k, m) = R**2*rtp2(i, k)
            wtrc_rtpthlp(i, k, m) = R*rtpthlp(i, k)
            wtrc_rtpthvp(i, k, m) = R*rtpthvp(i, k)

          end do
          do m = 2, wtrc_nwset
            R = wtrc_ratio(m, wtrc_rtm(i, k, m), wtrc_rtm(i, k, 1))
            wtrc_rtm(i, k, m) = R*rtm(i, k)
          end do
          wtrc_rtm(i, k, 1) = rtm(i, k)
        end if
        ...
        if (clubb_do_adv) then
          if (macmic_it .eq. 1) then
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_wprtp(i, k, m) = wprtp(i, k)*wtrc_ratio(m, wtrc_rtm(i, k, m), wtrc_rtm(i, k, 1))
                wtrc_rtpthlp(i, k, m) = rtpthlp(i, k)*wtrc_ratio(m, wtrc_rtm(i, k, m), wtrc_rtm(i, k, 1))
              end do
            end if
          end if
        end if
    ...
    if (clubb_do_adv) then
      ...
      if (trace_water) then
        wtrc_rtp2(1:ncol, pverp, :) = wtrc_rtp2(1:ncol, pver, :)
        wtrc_rtpthlp(1:ncol, pverp, :) = wtrc_rtpthlp(1:ncol, pver, :)
        wtrc_wprtp(1:ncol, pverp, :) = wtrc_wprtp(1:ncol, pver, :)
      end if
    end if
    ...
    do i = 1, ncol   ! loop over columns
      ...
      do k = 1, pverp
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            wtrc_rtm_in(k, m) = real(wtrc_rtm(i, pverp - k + 1, m), kind=core_rknd)
            wtrc_wprtp_in(k, m) = real(wtrc_wprtp(i, pverp - k + 1, m), kind=core_rknd)
            wtrc_rtp2_in(k, m) = real(wtrc_rtp2(i, pverp - k + 1, m), kind=core_rknd)
            wtrc_rtpthlp_in(k, m) = real(wtrc_rtpthlp(i, pverp - k + 1, m), kind=core_rknd)
            wtrc_rcm_inout(k, m) = real(wtrc_rcm(i, pverp - k + 1, m), kind=core_rknd)
            wtrc_rtpthvp_inout(k, m) = real(wtrc_rtpthvp(i, pverp - k + 1, m), kind=core_rknd)
          end do
        end if
      ...
      if (clubb_do_adv) then
        if (macmic_it .eq. 1) then
          ...
          do m = 1, wtrc_nwset
            ...
            wtrc_wprtp_in(:, m) = zt2zm_api(wtrc_wprtp_in(:, m))
            wtrc_rtp2_in(:, m) = zt2zm_api(wtrc_rtp2_in(:, m))
            wtrc_rtpthlp_in(:, m) = zt2zm_api(wtrc_rtpthlp_in(:, m))
          end do
          ...
          do k = 1, pverp
            ...
            do m = 1, wtrc_nwset
              wtrc_rtp2_in(k, m) = max(rt_tol**2, wtrc_rtp2_in(k, m))
            end do
          end do
        end if
      end if
      ...
      if (do_expldiff) then
        do k = 1, pver
          ...
          !water tracers:
          do m = 1, wtrc_nwset
            edsclr_in(k + 1, icnt + 2 + m) = wtrc_rtm(i, pver - k + 1, m)
          end do
        end do
      ...
      do t = 1, nadv    ! do needed number of "sub" timesteps for each CAM step
        ...
        call advance_clubb_core_api &
          (l_implemented, dtime, fcor, sfc_elevation, hydromet_dim, & ! intent(in)
           ...
           wpthvp_inout, wp2thvp_inout, rtpthvp_inout, wtrc_rtpthvp_inout, thlpthvp_inout, & ! intent(inout)
...
!                     rtpthlp_in = rtpthlp_in + rtpthlp_mc_out * dtime
      ...
      do k = 1, pverp
        ...
        !water tracers:
        do m = 1, wtrc_nwset
          wtrc_rtm(i, k, m) = real(wtrc_rtm_in(pverp - k + 1, m), kind=r8)
          wtrc_wprtp(i, k, m) = real(wtrc_wprtp_in(pverp - k + 1, m), kind=r8)
          wtrc_rtp2(i, k, m) = real(wtrc_rtp2_in(pverp - k + 1, m), kind=r8)
          wtrc_rtpthlp(i, k, m) = real(wtrc_rtpthlp_in(pverp - k + 1, m), kind=r8)
          wtrc_rtpthvp(i, k, m) = real(wtrc_rtpthvp_inout(pverp - k + 1, m), kind=r8)
          wtrc_rcm(i, k, m) = real(wtrc_rcm_inout(pverp - k + 1, m), kind=r8)
        end do
        ...
        do ixind = 1, pcnst
          if (lq(ixind)) then
            ...
            if ((ixind /= ixq) .and. (ixind /= ixcldliq) .and. &
                ...
                (ixind /= ixwp3) .and. (ixind /= ixup2) .and. &
                (ixind /= ixvp2) .and. (all(ixind /= wtrc_iatype(:, iwtvap))) .and. & !water tracers
                (all(ixind /= wtrc_iatype(:, iwtliq)))) then
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            do i = 1, ncol
              ...
              wtrc_wprtp(i, pverp, m) = wtrc_wprtp(i, pverp, m) + wprtp_const*R
              wtrc_rtpthlp(i, pverp, m) = wtrc_rtpthlp(i, pverp, m) + rtpthlp_const*R
            end do
          end do
        end if
      end if
...
!
    ...
    if (deep_scheme .ne. 'CLUBB_SGS') then
      if (relvar_fix) then
        where (rcm(:ncol, :pver) > qsmall .and. qclvar(:ncol, :pver) /= 0._r8) &
          relvar(:ncol, :pver) = min(relvarmax, max( &
                                     relvarmin, rcm(:ncol, :pver)**2/max(qsmall, &
                                                                         cloud_frac(:ncol, :pver)*qclvar(:ncol, :pver) - &
                                                                         (1._r8 - cloud_frac(:ncol, :pver))*rcm(:ncol, :pver)**2)))
        relvarc(:ncol, :pver) = min(relvarmax, max( &
                                    relvarmin, rcm(:ncol, :pver)**2/max(qsmall, &
                                                                        cloud_frac(:ncol, :pver)*qclvar(:ncol, :pver) - &
                                                                        (1._r8 - cloud_frac(:ncol, :pver))*rcm(:ncol, :pver)**2)))
      else
    ...
    do k = 1, pver
      do i = 1, ncol
        !  buoyancy flux
        wpthvp_diag(i, k) = (wpthlp(i, k) - (apply_const*wpthlp_const)) + &
                            ((1._r8 - eps)/eps)*real(theta0, kind=r8)* &
                            (wprtp(i, k) - (apply_const*wprtp_const)) + &
                            ((latvap/cpair)*state1%exner(i, k) - (1._r8/eps)*real(theta0, kind=r8))*wprcp(i, k)
...
!-------------------------------------------------------------------------------
    ...
    cflx(:ncol, :) = cam_in%cflx(:ncol, :)

    do m = 1, wtrc_nwset
      cflx(:ncol, wtrc_iatype(m, iwtvap)) = 0._r8
    end do

    do m = 2, pcnst

      ! ptend%q(:ncol,pver,m) = ptend%q(:ncol,pver,m) + tmp1(:ncol) * cam_in%cflx(:ncol,m)
      ptend%q(:ncol, pver, m) = ptend%q(:ncol, pver, m) + tmp1(:ncol)*cflx(:ncol, m)
    end do
