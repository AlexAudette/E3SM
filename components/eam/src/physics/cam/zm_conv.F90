...
contains
  ...
  subroutine zm_convr(lchnk, ncol, &
                      ...
                      c0mask, wtrpd, qds, wtevp, qhatb, evp_debug)
! WT-block ends
!-----------------------------------------------------------------------
!
! Purpose:
...
! transports can be done in outside of conv_cam.
    ...
    real(r8) :: wtrc_qtnd(pcols, pver, wtrc_nwset) !Specific humidity tendency for water tags (kg/kg/s) ! To be intent(out)
    real(r8) :: wtrc_dlf(pcols, pver, wtrc_nwset)    ! scattrd version of the detraining cld h2o tend   ! To be intent(out)
    real(r8) :: wtrc_pflx(pcols, pver, wtrc_nwset)  ! Water tracer precip flux at each level ! To be intent(out)
    real(r8) :: wtrc_qh(pcols, pver, wtrc_nwset) ! Grid slice of water tracer specific humidity ! To be intent(in)
    real(r8) :: wtrc_pflxg(pcols, pverp, wtrc_nwset) ! gather precip flux at each level for water tracers (local var)
...
! WT-block begins
    ...
    real(r8), intent(out) :: evp_debug(pcols, pver)    ! Debuggging
!----------------------------
! WT-block ends
...
!------------------------------Arguments--------------------------------
    ...
    real(r8), intent(inout) :: wtrc_prdprec(pcols, pver, pcnst)! precipitation production (kg/ks/s)
    ...
    real(r8), intent(inout) :: wtprec(pcols, pcnst)        ! Convective-scale preciptn rate
    real(r8), intent(out)   :: wtsnow(pcols, pcnst)        ! Convective-scale snowfall rate
!
!---------------------------Local storage-------------------------------
    ...
    real(r8) :: wtrc_fsnow_conv(pcols, pver, wtrc_nwset) ! snow fraction in precip production
...
!    real(r8), intent(out) :: substore(pcols,pver) !snow sublimation
    ...
    real(r8) :: wtrc_evpvint(pcols, wtrc_nwset)         ! vertical integral of evaporation

    real(r8) :: wtrc_evpprec(pcols, wtrc_nwset)         ! evaporation of precipitation (kg/kg/s)
    real(r8) :: wtrc_evpsnow(pcols, wtrc_nwset)         ! evaporation of snowfall (kg/kg/s)
    real(r8) :: wtrc_flxsntm(pcols, wtrc_nwset)         ! flux of snow into layer, after melting
    real(r8) :: wtrc_snowmlt(pcols, wtrc_nwset)         ! snow melt tendency in layer
...
! convert input precip to kg/m2/s
    ...
    wtprec(:ncol, :) = wtprec(:ncol, :)*1000_r8
    ...
    do k = 1, pver
      do i = 1, ncol
        ...
        if (t(i, k) > tmelt) then
          ...
          do m = 1, wtrc_nwset
            wtrc_flxsntm(i, m) = 0._r8
            wtrc_snowmlt(i, m) = wtrc_flxsnow(i, k, m)*gravit/pdel(i, k)
          end do
        else
          flxsntm(i) = flxsnow(i, k)
          snowmlt(i) = 0._r8
          do m = 1, wtrc_nwset
            wtrc_flxsntm(i, m) = wtrc_flxsnow(i, k, m)
            wtrc_snowmlt(i, m) = 0._r8
          end do
        end if

! relative humidity depression must be > 0 for evaporation
        evplimit = max(1._r8 - q(i, k, 1)/qs(i, k), 0._r8)
...
! flux prec is the net production above layer minus evaporation into environmet
        ...
        do m = 1, wtrc_nwset
          wtrc_evpprec(i, m) = ke*(1._r8 - cldfrc(i, k))*evplimit*sqrt(wtrc_flxprec(i, k, m))
        end do
...
! Currently does not include heating/cooling change to qs
        evplimit = max(0._r8, (qs(i, k) - q(i, k, 1))/deltat)
...
! Total evaporation cannot exceed input precipitation
        ...
        do m = 1, wtrc_nwset
          wtrc_evpprec(i, m) = wtrc_ratio(m, q(i, k, wtrc_iatype(m, iwtvap)), q(i, k, wtrc_iatype(1, iwtvap)))*evpprec(i)
        end do
...
!            prevent roundoff problems
        ...
        do m = 1, wtrc_nwset
          if (wtrc_flxprec(i, k, m) > 0._r8) then
            work1 = min(max(0._r8, wtrc_flxsntm(i, m)/wtrc_flxprec(i, k, m)), 1._r8)
            wtrc_evpsnow(i, m) = wtrc_evpprec(i, m)*work1
          else
            wtrc_evpsnow(i, m) = 0._r8
          end if
        ...
        do m = 1, wtrc_nwset
          wtrc_evpvint(i, m) = wtrc_evpvint(i, m) + wtrc_evpprec(i, m)*pdel(i, k)/gravit
          wtrc_ntprprd(i, k, m) = wtrc_prdprec(i, k, wtrc_iatype(m, iwtvap)) - wtrc_evpprec(i, m)
        end do
...
!         work2 = fsnow_conv(i,k)
        ...
        do m = 1, wtrc_nwset
          if (wtrc_flxprec(i, k, m) .gt. 0._r8) then
            work1 = min(max(0._r8, wtrc_flxsnow(i, k, m)/wtrc_flxprec(i, k, m)), 1._r8)
          else
            work1 = 0._r8
          end if
          work2 = max(fsnow_conv(i, k), work1)
          if (wtrc_snowmlt(i, m) .gt. 0._r8) work2 = 0._r8
          !         work2 = fsnow_conv(i,k)
          wtrc_ntsnprd(i, k, m) = wtrc_prdprec(i, k, wtrc_iatype(m, iwtvap))*work2 - wtrc_evpsnow(i, m) - wtrc_snowmlt(i, m)
        end do
...
! protect against rounding error
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            tend_q(i, k, wtrc_iatype(m, iwtvap)) = wtrc_evpprec(i, m)
          end do
        end if
    ...
    if (trace_water) then
      do m = 1, wtrc_nwset
        wtprec(:ncol, wtrc_iatype(m, iwtvap)) = wtrc_flxprec(:ncol, pver + 1, m)/1000._r8
        wtsnow(:ncol, wtrc_iatype(m, iwtvap)) = wtrc_flxsnow(:ncol, pver + 1, m)/1000._r8
      end do
    end if
...
!
          dboydt(i, k) = ((dtpdt(i, k)/tp(i, k) + 1._r8/(1._r8 + 1.608_r8*qstp(i, k) - q(i, mx(i)))* &
                          ...
                          (dtmdt(i, k)/t(i, k) + &
                           0.608_r8/(1._r8 + 0.608_r8*q(i, k))*dqmdt(i, k)))*grav*thetavp(i, k)/thetavm(i, k)
        end if
      end do
    end do
!
