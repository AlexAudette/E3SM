...
! Water tracer processing
    ...
    public :: wtrc_q1q2_pjr_cam5         ! calculates water tracer tendency from ZM deep scheme.
            ...
            do icol = 1, ncol
                  do itype = 1, pwtype
                    ...
                    pstate%q(icol, k, wtrc_iawset(itype, 1)) = pstate%q(icol, k, wtrc_bulk_indices(itype))
                      ...
                      if (debug_rescaler) then
                        qtmp = 0._r8
                        do iwset = 3, 20
                          qtmp = qtmp + pstate%q(icol, k, wtrc_iawset(itype, iwset))
                        enddo

                        if (abs(qtmp - pstate%q(icol, k, wtrc_iawset(itype, 1))) .gt. 1.e-16) then
                          write(iulog, *) qtmp,  pstate%q(icol, k, wtrc_iawset(itype, 1))
                          call endrun('rescaling does not work properly')
                        endif
                      endif
...
!-----------------------------------------------------------------------
        ...
        wtrc_fixed_rstd(1) = 1.0_r8
        if (wtrc_nwset .ge. 2) then
          do iwset = 2, 19
            wtrc_fixed_rstd(iwset) = 1.0_r8/18._r8
          enddo
          do iwset = 20, wtrc_nwset
            wtrc_fixed_rstd(iwset) = 1.0_r8/36._r8
          enddo
        endif
...
do i = 1,ncol
  ...
  do p = 1,pwtype
      ...
      ! oval = state%q(i,k,wtrc_iatype(1,p))
      m=2  
      state%q(i,k,wtrc_iatype(m,p)) = state%q(i,k,wtrc_bulk_indices(p))/2._r8
      ! end if
  end do
  end do
end do
...
! !=======================================================================
! subroutine wtrc_tend_fixer(state,ptend)
!   !--------------------------------
!   !
!   !Purpose:  To reset the standard (H2O) water tracer back to the bulk water value, and adjust
!   !all of the proceeding water tracers bay an amount proportional to the H2O reset value.
...
!   type(physics_tend), intent(inout) :: ptend      ! Physics state variables
...
!           oval = state%q(i,k,wtrc_iatype(1,p))
!           do m=2,wtrc_nwset
!               if (abs(ptend%q(i,k,wtrc_iatype(m,p)) - ptend%q(i,k,wtrc_iatype(1,p)) ) .gt. 1e-24) then
!               !calculate ratio:
!               R = wtrc_ratio(m,state%q(i,k,wtrc_iatype(m,p)),&
!               state%q(i,k,wtrc_iatype(1,p)))
!               !Adjust value:
!               if (R .gt. 1._r8) then
!                   ptend%q(i,k,wtrc_iatype(m,p)) = ptend%q(i,k,wtrc_iatype(1,p))
!               else
!                   ptend%q(i,k,wtrc_iatype(m,p)) = R*ptend%q(i,k,wtrc_iatype(1,p))
!               endif
!             endif
!           end do
!       end do
...
!   end subroutine wtrc_tend_fixer
...
subroutine wtrc_mass_fixer_first_step(state)
    ...
    do i = 1,ncol
        do k = 1,pver
        do p = 1,pwtype
          oval = state%q(i,k,wtrc_iatype(1,1))
            do m = 1, wtrc_nwset
            ...
            do k = 1, pver
              do icol = 1, ncol
                    do itype = 1, pwtype
                        ...
                        iwset = 2
                        if (abs(qloc(icol, k, wtrc_iawset(itype, iwset)) - 0.5_r8*qloc(icol, k, wtrc_bulk_indices(itype))) .gt. max_diff) then
                            write(iulog, *) 'Comprehensive tendency wtrc_check_h2o does not add up for 1/2 tracer : '//testname, qloc(icol, k, wtrc_iawset(itype, iwset)) , 0.5_r8*qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                            if (.not. wtrc_warn_only) then
                                call endrun('Comprehensive tendency water content check failed for 1/2 tracer : '//testname)
                            end if 
                        end if
                        iwset = 1
                        if (abs(qloc(icol, k, wtrc_iawset(itype, iwset)) - qloc(icol, k, wtrc_bulk_indices(itype))) .gt. max_diff) then
                            write(iulog, *) 'Comprehensive tendency wtrc_check_h2o does not add up for full tracer : '//testname, qloc(icol, k, wtrc_iawset(itype, iwset)) , qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                            if (.not. wtrc_warn_only) then
                        ...
                        ! iwset = 3
                        ! if (qloc(icol, k, wtrc_iawset(itype, iwset)) - qloc(icol, k, wtrc_bulk_indices(itype)) .gt. max_diff) then
                        !     write(iulog, *) 'Comprehensive tendency wtrc_check_h2o does not add up for hemispheric tracer : '//testname, qloc(icol, k, wtrc_iawset(itype, iwset)) , qloc(icol, k, wtrc_bulk_indices(itype)), qloc(icol, k, wtrc_iawset(itype, iwset)) - qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                        !     if (.not. wtrc_warn_only) then
                        !         call endrun('Comprehensive tendency water content check failed for hemispheric tracer : '//testname)
                        !     end if 
                        ! end if
                        if (abs(qloc(icol, k, wtrc_iawset(itype, 3)) + qloc(icol, k, wtrc_iawset(itype, 4)) - qloc(icol, k, wtrc_bulk_indices(itype))) .gt. max_diff) then
                          write(iulog, *) 'Comprehensive wtrc_check_h2o does not add up for hemispheric tracer : '//testname,qloc(icol, k, wtrc_iawset(itype, 3)) + qloc(icol, k, wtrc_iawset(itype, 4)) , qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                          if (.not. wtrc_warn_only) then
            ...
            do icol = 1, ncol
                do k = 1, pver
                    do itype = 1, pwtype
                      ...
                      ! iwset = 2
                      ! if (abs(qloc(icol, k, wtrc_iawset(itype, iwset)) - .5_r8*qloc(icol, k, wtrc_bulk_indices(itype))) .gt. max_diff) then
                      !   write(iulog, *) 'Comprehensive wtrc_check_h2o does not add up for 1/2 tracer : '//testname, qloc(icol, k, wtrc_iawset(itype, iwset)) , 0.5_r8*qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                      !   if (.not. wtrc_warn_only) then
                      !     call endrun('Comprehensive water content check failed for 1/2 tracer : '//testname)
                      !   end if 
                      ! end if
                      
                      iwset = 1
                      if (abs(qloc(icol, k, wtrc_iawset(itype, iwset)) - qloc(icol, k, wtrc_bulk_indices(itype))) .gt. max_diff) then
                          write(iulog, *) 'Comprehensive wtrc_check_h2o does not add up for full tracer : '//testname, qloc(icol, k, wtrc_iawset(itype, iwset)) , qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                          if (.not. wtrc_warn_only) then
                        ...
                        ! iwset = 3
                        ! if (qloc(icol, k, wtrc_iawset(itype, iwset)) - qloc(icol, k, wtrc_bulk_indices(itype)) .gt. max_diff) then
                        !     write(iulog, *) 'Comprehensive wtrc_check_h2o does not add up for hemispheric tracer : '//testname, qloc(icol, k, wtrc_iawset(itype, iwset)) , qloc(icol, k, wtrc_bulk_indices(itype)), qloc(icol, k, wtrc_iawset(itype, iwset)) - qloc(icol, k, wtrc_bulk_indices(itype)), icol, k, wtrc_bulk_names(itype)
                        !     if (.not. wtrc_warn_only) then
                        !         call endrun('Comprehensive water content check failed for hemispheric tracer : '//testname)
                        !     end if 
                        ! end if
...
!=======================================================================
!=======================================================================
subroutine wtrc_q1q2_pjr_cam6(dqdt        , ideep, lengath, &
                    q       ,qub     , done   ,lel     ,lcl     , &
...
!ratios are conserved. - JN.
    ...
    do k = msg+2,pver
        ...
        if (k >= jd(i) .and. k < mx(i) .and. eps0(i) > 0._r8) then
            ...
            Rd(i,k,m) = wtrc_ratio(m,rmass(i,m),rmass(i,1))
...
return
end subroutine wtrc_q1q2_pjr_cam6
!=======================================================================
!Version 3
!=======================================================================
!=======================================================================
...
!***********************
    ...
    !         do i = 1, lengath
    !             do k = 1 , pver
    !                 Rr = wtrc_ratio(m,q(ideep(i),k,wtrc_iatype(m,iwtvap)),q(ideep(i),k,wtrc_iatype(1,iwtvap)))
    !                 wtrprd(ideep(i),k,wtrc_iatype(m,iwtvap)) = wtrprd(ideep(i),k,wtrc_iatype(1,iwtvap))* Rr
    !             enddo
    !         enddo
    !     enddo
    ! endif
...
!=======================================================================
!=======================================================================
subroutine wtrc_q1q2_pjr_old(dqdt        , ideep, lengath, &
    q       ,qub     , done   ,lel     ,lcl     , &
    dupc    ,du      ,dp      ,qstb    ,qsthatb , &
    hmn     ,hsatb   ,hsthatb ,eu      , &
    mu      ,md      ,qdb     ,tu      ,td      , &
...
real(r8), intent(in) :: hmn(pcols,pver)     !bulk environmental moist static energy
    ...
    real(r8) qu(pcols,pver,wtrc_nwset)    !water vapor mixing ratio in updraft [kg/kg]
    real(r8) qd(pcols,pver,wtrc_nwset)    !water vapor mixing ratio in downdraft [kg/kg]
    real(r8) wtcu(pcols,pver,wtrc_nwset)  !water tracer condensation rate [?]
    real(r8) wtevp(pcols,pver,wtrc_nwset) !water tracer evaporation rate [?]
    real(r8) hu(pcols,pver,wtrc_nwset)    !tracer moist static energy in updraft
    real(r8) hd(pcols,pver,wtrc_nwset)    !tracer moist static energy in downdraft
    real(r8) wthmn(pcols,pver,wtrc_nwset) !tracer environmental moist static energy
    real(r8) hsat(pcols,pver,wtrc_nwset)  !tracer saturated env. moist static energy
    real(r8) qst(pcols,pver,wtrc_nwset)   !tracer saturated env. vapor mixing ratio
    real(r8) gamma(pcols,pver,wtrc_nwset) !No clue, just need it for parameterization
    real(r8) hsthat(pcols,pver,wtrc_nwset)!hsat at interfaces 
    real(r8) qsthat(pcols,pver,wtrc_nwset)!qsat at interfaces 
    real(r8) gamhat(pcols,pver,wtrc_nwset)!Gamma at interfaces 
    ...
    !For precip production:
    real(r8) totpcp(pcols,wtrc_nwset)     !Total precipitation [?] 
    real(r8) totevp(pcols,wtrc_nwset)     !Total evaporation [?] 
            ...
            do i=1,lengath
                if (k >= jd(i) .and. k < mx(i) .and. eps0(i) > 0._r8) then
                    ...
                    do m=1,wtrc_nwset
                        ...
                        Rd(i,k,m) = wtrc_ratio(m,qd(i,k,m),qd(i,k,1)) !calculate tracer ratio
    ...
    return
end subroutine wtrc_q1q2_pjr_old


subroutine wtrc_q1q2_pjr_cam5(dqdt        , ideep, lengath, &
                    q       ,qub     , done   ,lel     ,lcl     , &
                    dupc    ,du      ,dp      ,qstb    ,qsthatb , &
                    hmn     ,hsatb   ,hsthatb ,eu      , &
                    mu      ,md      ,qdb     ,tu      ,td      , &
...
!
   ...
   real(r8), intent(in) :: qsthatb(pcols,pver) !qst at interfaces
   real(r8), intent(in) :: qub(pcols,pver)     !bulk water vapor mixing ratio in updraft [kg/kg]
   real(r8), intent(in) :: qhatb(pcols,pver)     !bulk water vapor mixing ratio in updraft [kg/kg]
real(r8), intent(in) :: hmn(pcols,pver)     !bulk environmental moist static energy
real(r8), intent(in) :: hsatb(pcols,pver)   !bulk saturated env. moist static energy
real(r8), intent(in) :: hsthatb(pcols,pver) !hsat at interfaces
real(r8), intent(in) :: dp(pcols,pver)      !layer thickness [mb]
...
real(r8) totevp(pcols,wtrc_nwset)     !Total evaporation [?] 
real(r8) rprd(pcols,pver)                     !Precipitation production pre-unit change [?]
real(r8) rprdb(pcols,pver)                     !Precipitation production pre-unit change [?] base for debugging
   real(r8) pevpb(pcols,pver)                     !Precipitation production pre-unit change [?] base for debugging
!NOTE:  Might need to make "pevp" the evaporation term used in the actual
!tendency calculation! - JN
real(r8) pevp(pcols,pver)                     !precipitation evaporation 
real(r8) ql(pcols,pver,wtrc_nwset)            !precipitable liquid [kg/kg]
...
!Calculate ratio:
  ...
  Ru(i,k,m) = wtrc_ratio(m,qu(i,k+1,m),qu(i,k+1,1)) !calculate ratio
! if (m.eq. 2) then
            !   if (abs(Ru(i,k,m) - 0.5_r8) .gt. 1.e-24) then
            !     write(iulog, *) 'Ru error', Ru(i,k,m),qu(i,k+1,m),qu(i,k+1,1)
            !     call endrun('Ru error')
            !   endif
            ! endif
...
! if (m.eq. 1) then
            !   if (abs(wtcu(i,k,m) - cupc(i,k)) .gt. 1.e-24) then
            !     write(iulog, *) 'CU error', wtcu(i,k,m) , cu(i,k), Ru(i,k,m), cupc(i,k)
            !     call endrun('Cu error')
            !   endif
            ! endif
...
do m=1,wtrc_nwset
  ...
  Rd(i,k,m) = wtrc_ratio(m,qd(i,k,m),qd(i,k,1)) !calculate tracer ratio
!  if (m.eq. 2) then
          !   if (abs(Rd(i,k,m) - 0.5_r8) .gt. 1.e-24) then
          !     write(iulog, *) 'Rd error', Rd(i,k,m),qd(i,k,m),qd(i,k,1)
...
wtevp(i,k,m) = Rd(i,k,m)*evpc(i,k)
          ...
          !  if (m.eq. 2) then
          !   if (abs(wtevp(i,k,m) - 0.5_r8* evpc(i,k)) .gt. 1.e-24) then
          !     write(iulog, *) 'wtevp error', wtevp(i,k,m),Rd(i,k,m),evpc(i,k)*0.5_r8
...
wtrprd(ideep(i),k,wtrc_iatype(m,iwtvap)) = Rr*wtrprd(ideep(i),k,1)  !calculate precip production (done to avoid unit conversions)
          ...
          Rr = 1._r8
          wtrprd(ideep(i),k,wtrc_iatype(m,iwtvap)) =0.0_r8! Rr*wtrprd(ideep(i),k,1)  !calculate precip production (done to avoid unit conversions)
...
return
end subroutine wtrc_q1q2_pjr_cam5
