module physpkg
  ...
  use water_tracer_vars, only: trace_water, wtrc_bulk_indices, wtrc_iawset
...
!$OMP PRIVATE (c, beg_chnk_cnt, ncol, phys_buffer_chunk, end_chnk_cnt, sysclock_rate, sysclock_max, chunk_cost)
  ...
  subroutine tphysac(ztodt, cam_in, &
    ...
    ! Water tracers:
    use water_tracer_vars, only: trace_water, wtrc_iatype
    use water_tracers, only: wtrc_mass_fixer_first_step
...
!!!   call check_energy_chng(state, tend, "drymass", nstep, ztodt, zero, zero, zero, zero)
  ...
  subroutine tphysbc(ztodt, &
    ...
    use water_tracers, only: wtrc_get_rstd, &
                             wtrc_is_tagged, wtrc_mass_fixer, wtrc_mass_fixer_first_step, wtrc_ratio, &
                             wtrc_rescale_groups, wtrc_check_h2o_comp, wtrc_check_tend_comp
    use water_types, only: pwtype, iwtstrain, iwtstsnow, iwtliq, iwtvap, iwtice
    ...
    integer i, k, m, n, ihist                       ! Longitude, level, constituent indices
    integer :: ixcldice, ixcldliq              ! constituent indices for cloud liquid and ice water.
    ...
    real(r8), pointer :: wtsnow(:)      ! water tracer/isotope precipitation
    ...
    !***********************************
    !Correct water tracer masses
    !***********************************
    if (trace_water) then
      if (is_first_step()) then
        ! call wtrc_mass_fixer_first_step(state)
        call wtrc_mass_fixer(state)
        ! isok = wtrc_check_h2o_comp('after wtrc_mass_fixer_first_step', state, state%q, 1.0_r8)

      else
        call wtrc_mass_fixer(state)
        ! isok = wtrc_check_h2o_comp('after wtrc_mass_fixer', state, state%q, 1.0_r8)
      end if
    end if
    ...
    ! isok = wtrc_check_h2o_comp('after initial use_mass_borrower', state, state%q, 1.0_r8)
...
!!== KZ_WCON
    ...
    if (l_dry_adj) then
      ...
      if (trace_water) then
        do m = 1, wtrc_nwset
          do i = 1, ncol
            do k = 1, pver
              R = wtrc_ratio(m, state%q(i, k, wtrc_iatype(m, iwtvap)), state%q(i, k, wtrc_iatype(1, iwtvap)))
              ptend%q(i, k, wtrc_iatype(m, iwtvap)) = R*ptend%q(i, k, 1)
            end do
          end do
        end do
      end if
      ...
      ! isok = wtrc_check_h2o_comp('after dry_adjustment', state, state%q, 1.0_r8)
    ...
    !  if (trace_water) then
    !    call wtrc_mass_fixer_first_step(state)
    !    ! call wtrc_rescale_groups(state)
    !  endif

    ! isok = wtrc_check_h2o_comp('before convect_deep_tend', state, state%q, 1.0_r8)
    ...
    call physics_update(state, ptend, ztodt, tend)
    ! isok = wtrc_check_h2o_comp('after convect_deep_tend', state, state%q, 1.0_r8)
    ...
    ! isok = wtrc_check_h2o_comp('after convect_shallow_tend', state, state%q, 1.0_r8)
...
!!== KZ_WATCON
            ...
            ! isok = wtrc_check_h2o_comp('before clubb', state, state%q, 1.0_r8)

            ! isok = wtrc_check_tend_comp('after clubb_tend', state, ptend%q, 1.0_r8)
            ...
            !  call wtrc_mass_fixer_first_step(state)
            ! isok = wtrc_check_h2o_comp('after clubb', state, state%q, 1.0_r8)
        ...
        if (l_st_mic) then
          ...
          ! (see above note for macrophysics).
          call physics_ptend_scale(ptend, 1._r8/cld_macmic_num_steps, ncol)
          ! isok = wtrc_check_tend_comp('after microp_tend', state, ptend%q, 1.0_r8)

          call physics_update(state, ptend, ztodt, tend)
          ! call wtrc_mass_fixer(state)
          ! isok = wtrc_check_h2o_comp('after microp_tend', state, state%q, 1.0_r8)
...
!water tracers:
    ...
    ! isok = wtrc_check_h2o_comp('after tphysbc', state, state%q, 1.0_r8)
