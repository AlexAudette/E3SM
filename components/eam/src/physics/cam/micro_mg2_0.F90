...
!microphysics routine for each timestep goes here...
  ...
  subroutine micro_mg_tend( &
    ...
    wtrc_qvn, wtrc_qcn, wtrc_qin, &
    wtrc_qrn, wtrc_qsn, &
    wtrc_qvlat, wtrc_qctend, &
    wtrc_qitend, wtrc_qrtend, &
    wtrc_qstend, wtrc_prect, &
    wtrc_preci)
    ...
    use water_tracer_vars, only: wtrc_nwset, trace_water, wtrc_iawset, iwspec
    ...
    ! Water tag variables
    real(r8), intent(in) :: wtrc_qvn(mgncol, nlev, wtrc_nwset)        ! water tag vapour mixing ratio
    real(r8), intent(in) :: wtrc_qcn(mgncol, nlev, wtrc_nwset)        ! water tag liquid mixing ratio
    real(r8), intent(in) :: wtrc_qin(mgncol, nlev, wtrc_nwset)        ! water tag ice mixing ratio
    real(r8), intent(in) :: wtrc_qrn(mgncol, nlev, wtrc_nwset)        ! water tag rain mixing ratio
    real(r8), intent(in) :: wtrc_qsn(mgncol, nlev, wtrc_nwset)        ! water tag snow mixing ratio

    real(r8) :: wtrc_qv(mgncol, nlev, wtrc_nwset)        ! water tag vapour mixing ratio
    real(r8) :: wtrc_qc(mgncol, nlev, wtrc_nwset)        ! water tag liquid mixing ratio
    real(r8) :: wtrc_qi(mgncol, nlev, wtrc_nwset)        ! water tag ice mixing ratio
    real(r8) :: wtrc_qr(mgncol, nlev, wtrc_nwset)        ! water tag rain mixing ratio
    real(r8) :: wtrc_qs(mgncol, nlev, wtrc_nwset)        ! water tag snow mixing ratio

    real(r8), intent(out) :: wtrc_qvlat(:, :, :)     ! water tag vapour tendency
    real(r8), intent(out) :: wtrc_qctend(:, :, :)    ! water tag liquid tendency
    real(r8), intent(out) :: wtrc_qitend(:, :, :)    ! water tag ice tendency
    real(r8), intent(out) :: wtrc_qrtend(:, :, :)    ! water tag rain tendency
    real(r8), intent(out) :: wtrc_qstend(:, :, :)    ! water tag snow tendency
    real(r8), intent(out) :: wtrc_prect(:, :)          ! surface precip rate (m/s)
    real(r8), intent(out) :: wtrc_preci(:, :)          ! cloud ice/snow precip rate (m/s)
    ...
    real(r8) :: wtrc_qtmp(mgncol, nlev, wtrc_nwset)      ! mass tendency from freezing of rain (kg/kg/s)

    real(r8) :: frzro(mgncol, nlev)      ! mass tendency from freezing of rain (kg/kg/s)
    real(r8) :: meltso(mgncol, nlev)     ! mass tendency from melting of snow (kg/kg/s)
    real(r8) :: frzrpst(mgncol, nlev)    ! mass tendency from rain freezing post-sedimentation
    real(r8) :: meltspst(mgncol, nlev)   ! mass tendency from snow melting post-sedimentation
    real(r8) :: wtprelat(mgncol, nlev)   ! latent heating rate due to pre-sed processes
    real(r8) :: wtsedlat(mgncol, nlev)   ! latent heating rate during sedimentation
    real(r8) :: wtpostlat(mgncol, nlev)  ! latent heating  due to post-sed processes
    real(r8) :: wtfri_pre(mgncol, nlev)  ! Is freezing rain being added to cloud ice?
    real(r8) :: wtfri_post(mgncol, nlev) ! Is freezing rain post-sedimentation being added to cloud ice?
    real(r8) :: wtfc(mgncol, nlev)       ! cloud liquid sedimentation fall rate
    real(r8) :: wtfi(mgncol, nlev)       ! cloud ice sedimentation fall rate
    real(r8) :: wtfr(mgncol, nlev)       ! stratiform rain sedimentation fall rate
    real(r8) :: wtfs(mgncol, nlev)       ! stratiform snow sedimentation fall rate
    !--ag
    ...
    real(r8) :: wtrc_faloutc(nlev, wtrc_nwset)
    real(r8) :: wtrc_falouti(nlev, wtrc_nwset)

    real(r8) :: wtrc_faloutr(nlev, wtrc_nwset)
    real(r8) :: wtrc_falouts(nlev, wtrc_nwset)
    ...
    real(r8) :: wtrc_faltndc(wtrc_nwset)
    real(r8) :: wtrc_faltndi(wtrc_nwset)
    real(r8) :: wtrc_faltndqie(wtrc_nwset)
    real(r8) :: wtrc_faltndqce(wtrc_nwset)
    real(r8) :: wtrc_faltndr(wtrc_nwset)
    real(r8) :: wtrc_faltnds(wtrc_nwset)
    ...
    real(r8) :: wtrc_dumc(mgncol, nlev, wtrc_nwset)   ! qc
    real(r8) :: wtrc_dumi(mgncol, nlev, wtrc_nwset)   ! qi
    real(r8) :: wtrc_dumr(mgncol, nlev, wtrc_nwset)   ! rain mixing ratio
    real(r8) :: wtrc_dums(mgncol, nlev, wtrc_nwset)   ! snow mixing ratio
    ...
    real(r8), dimension(mgncol, nlev, wtrc_nwset):: vap_ratio_array, ice_ratio_array, &
                                                    liq_ratio_array, rain_ratio_array, snow_ratio_array

    ! loop array variables
    ! "i" and "k" are column/level iterators for internal (MG) variables
    ! "n" is used for other looping (currently just sedimentation)
    integer i, k, n, m ! might already be defined (m)
    ...
    micro_vert_loop: do k = 1, nlev
      ...
      call kk2000_liq_autoconversion(microp_uniform, qcic(:, k), &
                                     ncic(:, k), rho(:, k), relvar(:, k), mg_prc_coeff_fix, prc_coef1, prc_exp, &
                                     prc_exp1, prc(:, k), nprc(:, k), nprc1(:, k))
      ...
      if (do_cldice) then
        ...
        do i = 1, mgncol
          ...
          dum = ((-mnuccc(i, k) - mnucct(i, k) - mnudep(i, k) - msacwi(i, k))* &
                 lcldm(i, k) + (prci(i, k) + &
      ...
      if (do_cldice) then
        ...
        do i = 1, mgncol
          ...
          dum = ((-nnucct(i, k) - tmpfrz - nnudep(i, k) - nsacwi(i, k))* &
                 lcldm(i, k) + (nprci(i, k) + &
      ...
      do i = 1, mgncol
        ...
        tlat(i, k) = tlat(i, k) + ((pre(i, k)*precip_frac(i, k))*xxlv + &
                                   (prds(i, k)*precip_frac(i, k) + vap_dep(i, k) + ice_sublim(i, k) + &
                                    mnuccd(i, k) + mnudep(i, k)*lcldm(i, k))*xxls + &
                                   ((bergs(i, k) + psacws(i, k) + mnuccc(i, k) + mnucct(i, k) + &
                                     msacwi(i, k))*lcldm(i, k) + &
                                    (mnuccr(i, k) + pracs(i, k) + mnuccri(i, k))*precip_frac(i, k) + &
                                    berg(i, k))*xlf)
        ...
        qstend(i, k) = qstend(i, k) + &
                       (prai(i, k) + prci(i, k))*icldm(i, k) + (psacws(i, k) + bergs(i, k))*lcldm(i, k) + &
        ...
        if (trace_water) then
          ...
          do m = 1, wtrc_nwset
            ...
            wtrc_qvlat(i, k, m) = wtrc_qvlat(i, k, m) - &
                                  (pre(i, k)*rain_ratio_array(i, k, m) + &
                                   prds(i, k)*snow_ratio_array(i, k, m))*precip_frac(i, k) - &
                                  vap_dep(i, k)*vap_ratio_array(i, k, m) - &
                                  ice_sublim(i, k)*ice_ratio_array(i, k, m) - &
                                  mnuccd(i, k)*vap_ratio_array(i, k, m) - &
                                  mnudep(i, k)*lcldm(i, k)*vap_ratio_array(i, k, m)
            ...
            if (do_cldice) then
              ...
              wtrc_qitend(i, k, m) = wtrc_qitend(i, k, m) + &
                                     ((mnuccc(i, k) + mnucct(i, k) + msacwi(i, k))*liq_ratio_array(i, k, m) + &
                                      mnudep(i, k)*vap_ratio_array(i, k, m))*lcldm(i, k) + &
            ...
            wtrc_qrtend(i, k, m) = wtrc_qrtend(i, k, m) + &
                                   (pra(i, k) + prc(i, k))*lcldm(i, k)*liq_ratio_array(i, k, m) + &
                                   (pre(i, k) - pracs(i, k) - mnuccr(i, k) - &
                                    mnuccri(i, k))*precip_frac(i, k)*rain_ratio_array(i, k, m)

            wtrc_qstend(i, k, m) = wtrc_qstend(i, k, m) + &
                                   (prai(i, k) + prci(i, k))*icldm(i, k)*ice_ratio_array(i, k, m) + &
                                   (psacws(i, k) + bergs(i, k))*lcldm(i, k)*liq_ratio_array(i, k, m) + &
                                   (prds(i, k)*snow_ratio_array(i, k, m) + &
                                    pracs(i, k)*rain_ratio_array(i, k, m) + &
                                    mnuccr(i, k)*rain_ratio_array(i, k, m))*precip_frac(i, k)
          end do
        ...
        if (do_cldice) then
          ...
          nitend(i, k) = nitend(i, k) + nnuccd(i, k) + &
                         (nnucct(i, k) + tmpfrz + nnudep(i, k) + &
                          nsacwi(i, k))*lcldm(i, k) + (nsubi(i, k) - nprci(i, k) - &
    ...
    sed_col_loop: do i = 1, mgncol
      ...
      if (.not. precip_off) then
        ...
        do k = 1, nlev
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_dumc(i, k, m) = (wtrc_qc(i, k, m) + wtrc_qctend(i, k, m)*deltat)/lcldm(i, k)
              wtrc_dumi(i, k, m) = (wtrc_qi(i, k, m) + wtrc_qitend(i, k, m)*deltat)/icldm(i, k)
              wtrc_dumr(i, k, m) = (wtrc_qr(i, k, m) + wtrc_qrtend(i, k, m)*deltat)/precip_frac(i, k)
              wtrc_dums(i, k, m) = (wtrc_qs(i, k, m) + wtrc_qstend(i, k, m)*deltat)/precip_frac(i, k)
            end do
            do m = 1, wtrc_nwset
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_dumc(i, k, m) = (wtrc_qc(i, k, m) + wtrc_qctend(i, k, m)*deltat)
        ...
        do n = 1, nstep
          ...
          if (do_cldice) then
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_falouti(:, m) = fi*wtrc_dumi(i, :, m)
              end do
            end if
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_faltndi(m) = wtrc_falouti(k, m)/pdel(i, k)
              wtrc_qitend(i, k, m) = wtrc_qitend(i, k, m) - wtrc_faltndi(m)/nstep
              wtrc_dumi(i, k, m) = wtrc_dumi(i, k, m) - wtrc_faltndi(m)/nstep*deltat
            end do
          end if
          ...
          do k = 2, nlev
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_faltndqie(m) = (wtrc_falouti(k, m) - wtrc_falouti(k - 1, m))/pdel(i, k)
                wtrc_faltndi(m) = (wtrc_falouti(k, m) - dum1*wtrc_falouti(k - 1, m))/pdel(i, k)

                wtrc_qitend(i, k, m) = wtrc_qitend(i, k, m) - wtrc_faltndi(m)/nstep
                wtrc_qvlat(i, k, m) = wtrc_qvlat(i, k, m) - (wtrc_faltndqie(m) - wtrc_faltndi(m))/nstep
                wtrc_dumi(i, k, m) = wtrc_dumi(i, k, m) - wtrc_faltndi(m)*deltat/nstep
              end do
            end if
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_prect(i, m) = wtrc_prect(i, m) + max(wtrc_falouti(nlev, m)/g/real(nstep)/1000._r8, 0._r8)
              wtrc_preci(i, m) = wtrc_preci(i, m) + max(wtrc_falouti(nlev, m)/g/real(nstep)/1000._r8, 0._r8)
            end do
          end if
        ...
        do n = 1, nstep
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_faloutc(:, m) = fc*wtrc_dumc(i, :, m)
            end do
          end if
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_faltndc(m) = wtrc_faloutc(k, m)/pdel(i, k)
              wtrc_qctend(i, k, m) = wtrc_qctend(i, k, m) - wtrc_faltndc(m)/nstep
              wtrc_dumc(i, k, m) = wtrc_dumc(i, k, m) - wtrc_faltndc(m)*deltat/nstep
            end do
          end if
          do k = 2, nlev
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_faltndqce(m) = (wtrc_faloutc(k, m) - wtrc_faloutc(k - 1, m))/pdel(i, k)
                wtrc_faltndc(m) = (wtrc_faloutc(k, m) - dum*wtrc_faloutc(k - 1, m))/pdel(i, k)
                wtrc_qctend(i, k, m) = wtrc_qctend(i, k, m) - wtrc_faltndc(m)/nstep
                wtrc_qvlat(i, k, m) = wtrc_qvlat(i, k, m) - (wtrc_faltndqce(m) - wtrc_faltndc(m))/nstep
                wtrc_dumc(i, k, m) = wtrc_dumc(i, k, m) - wtrc_faltndc(m)*deltat/nstep
              end do
            end if
          end do
        ...
        do n = 1, nstep
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_faloutr(:, m) = fr*wtrc_dumr(i, :, m)
            end do
          end if
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_faltndr(m) = wtrc_faloutr(k, m)/pdel(i, k)
              wtrc_qrtend(i, k, m) = wtrc_qrtend(i, k, m) - wtrc_faltndr(m)/nstep
          ...
          do k = 2, nlev
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_faltndr(m) = (wtrc_faloutr(k, m) - wtrc_faloutr(k - 1, m))/pdel(i, k)
                ! add fallout terms to eulerian tendencies
                wtrc_qrtend(i, k, m) = wtrc_qrtend(i, k, m) - wtrc_faltndr(m)/nstep
                wtrc_dumr(i, k, m) = wtrc_dumr(i, k, m) - wtrc_faltndr(m)*deltat/real(nstep)
              end do
        ...
        do n = 1, nstep
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_falouts(:, m) = fs*wtrc_dums(i, :, m)
            end do
          end if
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_faltnds(m) = wtrc_falouts(k, m)/pdel(i, k)
              wtrc_qstend(i, k, m) = wtrc_qstend(i, k, m) - wtrc_faltnds(m)/nstep
              wtrc_dums(i, k, m) = wtrc_dums(i, k, m) - wtrc_faltnds(m)*deltat/real(nstep)
            end do
          end if
          ...
          do k = 2, nlev
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_faltnds(m) = (wtrc_falouts(k, m) - wtrc_falouts(k - 1, m))/pdel(i, k)
                ! add fallout terms to eulerian tendencies
                wtrc_qstend(i, k, m) = wtrc_qstend(i, k, m) - wtrc_faltnds(m)/nstep
                wtrc_dums(i, k, m) = wtrc_dums(i, k, m) - wtrc_faltnds(m)*deltat/real(nstep)
              end do
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              wtrc_prect(i, m) = wtrc_prect(i, m) + max(wtrc_falouts(nlev, m)/g/real(nstep)/1000._r8, 0._r8)
      ...
      do k = 1, nlev
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            wtrc_dumc(i, k, m) = max(wtrc_qc(i, k, m) + wtrc_qctend(i, k, m)*deltat, 0._r8)
            wtrc_dumi(i, k, m) = max(wtrc_qi(i, k, m) + wtrc_qitend(i, k, m)*deltat, 0._r8)
            wtrc_dumr(i, k, m) = max(wtrc_qr(i, k, m) + wtrc_qrtend(i, k, m)*deltat, 0._r8)
            wtrc_dums(i, k, m) = max(wtrc_qs(i, k, m) + wtrc_qstend(i, k, m)*deltat, 0._r8)
          end do
          do m = 1, wtrc_nwset
        ...
        if (t(i, k) + tlat(i, k)/cpp*deltat > snowmelt) then
          if (dums(i, k) > 0._r8) then
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_qstend(i, k, m) = wtrc_qstend(i, k, m) - dum*wtrc_dums(i, k, m)/deltat
                wtrc_qrtend(i, k, m) = wtrc_qrtend(i, k, m) + dum*wtrc_dums(i, k, m)/deltat
              end do
            end if
        ...
        if (t(i, k) + tlat(i, k)/cpp*deltat < rainfrze) then
          ...
          if (dumr(i, k) > 0._r8) then
            ...
            if (lamr(i, k) < 1._r8/Dcs) then
              ...
              if (trace_water) then
                do m = 1, wtrc_nwset
                  wtrc_qstend(i, k, m) = wtrc_qstend(i, k, m) + dum*wtrc_dumr(i, k, m)/deltat
                end do
              end if
        ...
        if (t(i, k) + tlat(i, k)/cpp*deltat < 233.15_r8) then
          if (dumc(i, k) > 0._r8) then
            ...
            if (trace_water) then
              do m = 1, wtrc_nwset
                wtrc_qitend(i, k, m) = wtrc_qitend(i, k, m) + dum*wtrc_dumc(i, k, m)/deltat
              end do
            end if
        ...
        if (trace_water) then
          wtrc_qtmp = wtrc_qv + wtrc_qvlat*deltat
        end if
        ! use rhw to allow ice supersaturation
        call qsat_water(ttmp, p(i, k), esn, qvn)
        ...
        if (qtmp > qvn .and. qvn > 0 .and. allow_sed_supersat) then
          ...
          if (trace_water) then
            do m = 1, wtrc_nwset
              vap_ratio_array(i, k, m) = wtrc_ratio(m, wtrc_qtmp(i, k, m), wtrc_qtmp(i, k, 1))
              wtrc_qctend(i, k, m) = wtrc_qctend(i, k, m) + dum*(1._r8 - dum1)*vap_ratio_array(i, k, m)
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            wtrc_dumc(i, k, m) = max(wtrc_qc(i, k, m) + wtrc_qctend(i, k, m)*deltat, 0._r8)/lcldm(i, k)
            wtrc_dumi(i, k, m) = max(wtrc_qi(i, k, m) + wtrc_qitend(i, k, m)*deltat, 0._r8)/icldm(i, k)
            wtrc_dumr(i, k, m) = max(wtrc_qr(i, k, m) + wtrc_qrtend(i, k, m)*deltat, 0._r8)/precip_frac(i, k)
            wtrc_dums(i, k, m) = max(wtrc_qs(i, k, m) + wtrc_qstend(i, k, m)*deltat, 0._r8)/precip_frac(i, k)
          end do
          do m = 1, wtrc_nwset
            liq_ratio_array(i, k, m) = wtrc_ratio(m, wtrc_dumc(i, k, m), wtrc_dumc(i, k, 1))
            ice_ratio_array(i, k, m) = wtrc_ratio(m, wtrc_dumi(i, k, m), wtrc_dumi(i, k, 1))
        ...
        if (trace_water) then
          do m = 1, wtrc_nwset
            ! limit in-cloud mixing ratio to reasonable value of 5 g kg-1
            wtrc_dumc(i, k, m) = min(wtrc_dumc(i, k, m), 5.e-3_r8*liq_ratio_array(i, k, m))
            wtrc_dumi(i, k, m) = min(wtrc_dumi(i, k, m), 5.e-3_r8*ice_ratio_array(i, k, m))
            ! limit in-precip mixing ratios
            wtrc_dumr(i, k, m) = min(wtrc_dumr(i, k, m), 10.e-3_r8*rain_ratio_array(i, k, m))
            wtrc_dums(i, k, m) = min(wtrc_dums(i, k, m), 10.e-3_r8*snow_ratio_array(i, k, m))
          end do
        end if
    ...
    if (trace_water) then
      wtrc_qc = wtrc_qc + wtrc_qctend*deltat
      wtrc_qi = wtrc_qi + wtrc_qitend*deltat
    end if
