...
contains
  ...
  subroutine Allocation1_PlantNPDemand (bounds, num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    ! LOCAL VARAIBLES for sugarcane allocation based on Colmanetti et al., 2024
    ...
    real(r8), parameter :: sf3        = 0.009_r8 ! parameters for linear carbon allocation to the stalk-sucrose
    real(r8), parameter :: ipf3       = 5.0_r8   ! parameters for linear carbon allocation to the stalk-sucrose
    real(r8), parameter :: ecf4       = 0.025_r8 ! parameters for logarithmic carbon allocation to the stalk-sucrose
    real(r8), parameter :: ipf4       = 5.0_r8   ! parameters for logarithmic carbon allocation to the stalk-sucrose
         ...
         if (iscft(ivt(p))) then ! skip 2 generic crops
            ...
            if (croplive(p) .and. percrop(ivt(p)) == 0.0_r8 ) then
               ...
               if (ivt(p)==nsugarcane .or. ivt(p)==nsugarcaneirrig) then
                  ...
                  rmat = 100.0_r8 * (hui(p)/gddmaturity(p))

                  if (nyrs_crop_active(p) == 0) then ! Year 1
                     aerial(p) = (1.0_r8 - arootf(ivt(p))) * min(1.0_r8, (1 - exp(-(rootd * 0.6 * rmat))))
                  else
                     aerial(p) = (1.0_r8 - arootf(ivt(p))) * min(1.0_r8, (1 - exp(-(rootd * rmat))))
                  end if
                  ...
                  astem(p) = min( (1.0_r8 - aleaff(ivt(p)) - arootf(ivt(p))), (aerial(p) * max(af1, af2)) )
                  aleaf(p) = aerial(p) - astem(p)
